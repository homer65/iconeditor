...
import java.util.*;
public class ScalableIconPanel extends IconPanel implements MouseListener,MouseMotionListener
{
	...
	public void setIcon(Icon icon) 
	{
		...
		picon = ir.resize(picon,Parameter.iconPanelSize,Parameter.iconPanelSize);
		dim = picon.getSize();
		pwidth = dim.width;
		pheight = dim.height;
		faktorx = ((float) owidth) / ((float) pwidth);
		faktory = ((float) oheight) / ((float) pheight);
		setPreferredSize(new Dimension(Parameter.iconPanelSize,Parameter.iconPanelSize));
		repaint();
	}
